...
function gateKeeper(req, res, next) {
  ...
  const matchedUser = USERS.find(userfile => userfile.userName === user && userfile.password === pass)
  if(matchedUser){
...
let portLocation = process.env.PORT || 3005

app.listen(portLocation, () => {
  console.log(`Your app is listening on port ${portLocation}`);
});
